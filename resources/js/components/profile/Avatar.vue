<template>
  <div class="media">
    <img class="rounded-circle mr-4" style="width: 120px; height: 120px" :src="user.image_url" alt="user-avatar">
    <div class="media-body pt-1">
      <vue-clip
        ...
        <template slot="clip-uploader-action">
            <button class="dz-message btn btn-outline-secondary btn-sm" type="button">Scegli file</button>
        </template>
      </vue-clip>
      <small class="form-text text-muted">La massima grandezza concessa è di 10MB.</small>
...
export default {
  ...
  methods: {
    ...
    completeAvatar (file, status, xhr) {
      if (status === 'success') {
        this.user.image_url = JSON.parse(xhr.response).image_url
